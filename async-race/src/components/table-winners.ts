...
export default class Table extends BaseElement<HTMLTableElement>{
  constructor(){
    super('table', ['table']);
    this.createTableTitle();
    this.createNewRow(1, 'tesla', '#BDEEEE', 1, 10)
  }
  createTableTitle(){
    const tableTitleContainer = new BaseElement<HTMLTableRowElement>('tr', ['table-title']);
    tableTitleContainer.container.innerHTML = `
    <th>Number</th>
    <th>Car</th>
    <th>Name</th>
    `
    const winsRow = new BaseElement<HTMLTableSectionElement>('th', ['wins-row'], 'Wins');
    const timeRow = new BaseElement<HTMLTableSectionElement>('th', ['time-row'], 'best time, seconds');
    tableTitleContainer.container.appendChild(winsRow.container);
    tableTitleContainer.container.appendChild(timeRow.container);
    this.container.append(tableTitleContainer.container);
  }

  createNewRow(number:number, name: string, color: string, wins: number, time: number){
    const rowContainer = new BaseElement<HTMLTableRowElement>('tr', ['table-row']);
    rowContainer.container.innerHTML = `
    <td>${number}</td>
   ...
   61 239 98 16 10 -216 242 -234 235z"/>
    ...
    <td>${wins}</td>
    <td>${time}</td>`
    this.container.append(rowContainer.container);
  }
}