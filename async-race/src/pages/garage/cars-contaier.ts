...
export default class Car extends BaseElement<HTMLElement>{
  ...
  constructor(){
    ...
    this.initCarImage();
    const flagImage = new BaseElement<HTMLImageElement>('svg', ['flag']);
    flagImage.container.innerHTML = `<svg width="60" height="68" viewBox="0 0 60 68" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M37.6 8L36 0H0V68H8V40H30.4L32 48H60V8H37.6Z" fill="black"/>
    </svg>`;
    this.container.appendChild(flagImage.container);
  }

  createContentContainer(){
    const contentContainer = new BaseElement<HTMLDivElement>('div', ['content-container']);
    const buttonSelect = new Button(['button-select'], 'select');
    const buttonRemove = new Button(['button-remove'], 'remove');
    const carName = new BaseElement<HTMLElement>('p', ['car-name'], 'tesla');
    contentContainer.container.appendChild(buttonSelect.container);
    contentContainer.container.appendChild(buttonRemove.container);
    contentContainer.container.appendChild(carName.container);
    this.container.appendChild(contentContainer.container);
  }

  createButtonContainer(){
    const buttonContainer = new BaseElement<HTMLDivElement>('div', ['buttons-container']);
    const buttonStart = new Button(['button-start']);
    const buttonStop = new Button(['button-stop']);
    buttonContainer.container.appendChild(buttonStart.container);
    buttonContainer.container.appendChild(buttonStop.container);
    this.container.appendChild(buttonContainer.container);
  }

  initCarImage(){
    const imageContainer =new BaseElement<HTMLDivElement>('div', ['image-container']);
    imageContainer.container.innerHTML = `<svg version="1.0" xmlns="http://www.w3.org/2000/svg"
   ...
   61 239 98 16 10 -216 242 -234 235z"/>
    ...
    this.container.appendChild(imageContainer.container);
  }
}